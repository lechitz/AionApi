...
func (s *Service) SoftDeleteUser(ctx context.Context, userID uint64) error {
	if err := s.userStore.SoftDeleteUser(ctx, userID); err != nil {
		s.logger.Errorw(constants.ErrorToSoftDeleteUser, commonkeys.Error, err.Error())
		return err
	}
	...
	s.logger.Infow(constants.SuccessUserSoftDeleted, commonkeys.UserID, strconv.FormatUint(userID, 10))
	return nil
}
