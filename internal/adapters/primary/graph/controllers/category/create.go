...
func (h *Handler) Create(ctx context.Context, in model.CreateCategoryInput, userID uint64) (*model.Category, error) {
	...
	ctx, span := tracer.Start(ctx, constants.SpanCreate)
	defer span.End()
	...
	if err != nil {
		...
		h.Logger.ErrorwCtx(ctx, "error creating category",
	...
	span.SetStatus(codes.Ok, constants.StatusCreated)

	ip, _ := ctx.Value(ctxkeys.RequestIP).(string)
	ua, _ := ctx.Value(ctxkeys.RequestUserAgent).(string)
	h.Logger.InfowCtx(ctx, constants.MsgCreated,
		commonkeys.UserID, strconv.FormatUint(userID, 10),
