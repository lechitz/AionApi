// Package handler implements the GraphQL controller (handler) for Category operations.
// It orchestrates tracing, structured logging, and mapping, then delegates business logic
// to the core use case via the input ports (CategoryService).
package handler
...
func NewHandler(categoryService input.CategoryService, logger logger.ContextLogger) *Handler {
