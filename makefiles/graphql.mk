...
graphql.queries:
	...
	@printf 'query DashboardSnapshot($$date: String!, $$timezone: String) { dashboardSnapshot(date: $$date, timezone: $$timezone) { date timezone metrics { metricKey value previousValue delta trend valueType format precision sparklinePoints comparisonLabel metadata } goals { goalId title current target unit progress percentage status metricKey valueType format precision dueDate metadata } quickInsights healthScore activeGoals criticalGoals streakDays summary } }\n' > "$(QUERIES_DIR)/dashboard/snapshot.graphql"
	@printf 'query MetricDefinitions { metricDefinitions { id metricKey name description icon valueType format precision defaultPeriod defaultAggregation defaultVisible goalDefault displayOrder metadata isActive createdAt updatedAt } }\n' > "$(QUERIES_DIR)/dashboard/metric-definitions.graphql"
	@printf 'query DashboardViews { dashboardViews { id name description isDefault orderIndex widgets { id metricDefinitionId widgetType size orderIndex titleOverride configJson isActive createdAt updatedAt metricDefinition { id metricKey name description icon valueType format precision defaultPeriod defaultAggregation defaultVisible goalDefault displayOrder metadata isActive createdAt updatedAt } } createdAt updatedAt } }\n' > "$(QUERIES_DIR)/dashboard/views.graphql"
	@printf 'query DashboardView($$id: ID!) { dashboardView(id: $$id) { id name description isDefault orderIndex widgets { id metricDefinitionId widgetType size orderIndex titleOverride configJson isActive createdAt updatedAt metricDefinition { id metricKey name description icon valueType format precision defaultPeriod defaultAggregation defaultVisible goalDefault displayOrder metadata isActive createdAt updatedAt } } createdAt updatedAt } }\n' > "$(QUERIES_DIR)/dashboard/view.graphql"
	@printf 'query DashboardWidgetCatalog { dashboardWidgetCatalog { metricDefinitions { id metricKey name description icon valueType format precision defaultPeriod defaultAggregation defaultVisible goalDefault displayOrder metadata isActive createdAt updatedAt } goalTemplates { id title description metricKey targetValue unit dueDate daysToComplete colorHex icon priority isCompleted createdAt updatedAt } quickStats { totalMetrics activeGoals dueToday completionRate updatedAt } } }\n' > "$(QUERIES_DIR)/dashboard/widget-catalog.graphql"
	@printf 'query SuggestMetricDefinitions($$limit: Int) { suggestMetricDefinitions(limit: $$limit) { metricKey name description icon valueType format precision defaultPeriod defaultAggregation defaultVisible goalDefault displayOrder metadata } }\n' > "$(QUERIES_DIR)/dashboard/suggest-metric-definitions.graphql"
	...
	@printf 'mutation UpsertMetricDefinition($$input: UpsertMetricDefinitionInput!) { upsertMetricDefinition(input: $$input) { id metricKey name description icon valueType format precision defaultPeriod defaultAggregation defaultVisible goalDefault displayOrder metadata isActive createdAt updatedAt } }\n' > "$(MUTATIONS_DIR)/dashboard/upsert-metric-definition.graphql"
	@printf 'mutation UpsertGoalTemplate($$input: UpsertGoalTemplateInput!) { upsertGoalTemplate(input: $$input) { id title description metricKey targetValue unit dueDate daysToComplete colorHex icon priority isCompleted createdAt updatedAt } }\n' > "$(MUTATIONS_DIR)/dashboard/upsert-goal-template.graphql"
	...
	@printf 'mutation CreateDashboardView($$input: CreateDashboardViewInput!) { createDashboardView(input: $$input) { id name description isDefault orderIndex widgets { id metricDefinitionId widgetType size orderIndex titleOverride configJson isActive createdAt updatedAt } createdAt updatedAt } }\n' > "$(MUTATIONS_DIR)/dashboard/create-view.graphql"
	@printf 'mutation SetDefaultDashboardView($$input: SetDefaultDashboardViewInput!) { setDefaultDashboardView(input: $$input) { id name description isDefault orderIndex widgets { id metricDefinitionId widgetType size orderIndex titleOverride configJson isActive createdAt updatedAt } createdAt updatedAt } }\n' > "$(MUTATIONS_DIR)/dashboard/set-default-view.graphql"
	@printf 'mutation UpsertDashboardWidget($$input: UpsertDashboardWidgetInput!) { upsertDashboardWidget(input: $$input) { id metricDefinitionId widgetType size orderIndex titleOverride configJson isActive createdAt updatedAt metricDefinition { id metricKey name description icon valueType format precision defaultPeriod defaultAggregation defaultVisible goalDefault displayOrder metadata isActive createdAt updatedAt } } }\n' > "$(MUTATIONS_DIR)/dashboard/upsert-widget.graphql"
	@printf 'mutation ReorderDashboardWidgets($$input: ReorderDashboardWidgetsInput!) { reorderDashboardWidgets(input: $$input) { id metricDefinitionId widgetType size orderIndex titleOverride configJson isActive createdAt updatedAt metricDefinition { id metricKey name description icon valueType format precision defaultPeriod defaultAggregation defaultVisible goalDefault displayOrder metadata isActive createdAt updatedAt } } }\n' > "$(MUTATIONS_DIR)/dashboard/reorder-widgets.graphql"
	...
	@printf 'mutation CreateMetricAndWidget($$input: CreateMetricAndWidgetInput!) { createMetricAndWidget(input: $$input) { id metricDefinitionId widgetType size orderIndex titleOverride configJson isActive createdAt updatedAt metricDefinition { id metricKey name description icon valueType format precision defaultPeriod defaultAggregation defaultVisible goalDefault displayOrder metadata isActive createdAt updatedAt } } }\n' > "$(MUTATIONS_DIR)/dashboard/create-metric-and-widget.graphql"
